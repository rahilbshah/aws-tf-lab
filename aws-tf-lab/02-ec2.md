...
revision: revision/02-ec2-revision
tags: [topic, domain/resilient]
---

# 02 – EC2 (Elastic Compute Cloud)

The IaaS foundation of AWS: virtual servers (instances) you rent on AWS infrastructure with full OS control. One of AWS's founding services, launched in 2006 a few months after S3. Many other AWS services run on EC2 under the hood (RDS, EKS data plane, classic ECS) — abstracted away from you, but the compute is there.
...
One caveat the exam glosses over but your Terraform will not. That `false` for a data volume is the **console** default. AWS's own table splits it four ways: a data volume attached **at launch via the CLI/API** defaults to **Delete**, and only a data volume added **through the console at launch, or attached after launch**, defaults to Preserve. Terraform goes through the API — and `ebs_block_device.delete_on_termination` **defaults to `true`**. So an extra volume declared inline in `aws_instance` is destroyed with the instance, the opposite of what the table above would lead you to expect. Set it explicitly.
...
An **auto-assigned** public IP comes out of the AWS public pool. Whether you get one is normally the subnet's `map_public_ip_on_launch` (the default VPC's subnets have it on) — but **the launch-time setting, `associate_public_ip_address` on the instance, overrides the subnet in either direction.** It belongs to the running instance, not to you. Stop the instance and it's released; start again and you get a different address. It also changes across an instance-type change, because that change is a stop and a start.
...
---
**Self-test for this topic:** [[revision/02-ec2-revision#Self-test]]
